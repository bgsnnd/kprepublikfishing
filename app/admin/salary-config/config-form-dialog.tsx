...
export function ConfigFormDialog({
  ...
  // Component yang belum di-assign ke role ini
  const availableComponents = components.filter(
    (c) => !role.configs.some((cfg) => cfg.componentId === c.id),
  )
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success(json.data?.message ?? 'Config ditambahkan')
      onOpenChange(false)
      router.refresh()
    } catch {
        ...
        {availableComponents.length === 0 ? (
          <div className="rounded-lg border bg-muted/30 p-4 text-center">
            <p className="text-sm text-muted-foreground">
              Semua komponen aktif udah di-assign ke role ini.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger>
                  {/* ✅ FIX: render nama komponen, bukan ID */}
